...
$('div.elmntStr').each(function() {
	$(this).css("width",$(this).parent().width()-$(this).next().width()-8)
	})
}
...
setTimeout( function() {
getOptUpload();
if ($('#selsections2').size()>0) getSlect('selsections2',2);
if ($('#selsections3').size()>0) getSlect('selsections3',maxlevs);
...
ref=ref.split("#");
var obj=$('#submenu_container'+ref);
seturl="getLevel.asp?load="+ref[0]+"&lev="+ref[1];
if (obj.length>0)
{
...
gIndex=gIndex+1;
	if (gIndex<=openedStructure.length) openLevels(gIndex);
		setTimeout(function() { setLimgs() },1200)
...
gH=$(window).height()-$('#Editor_tmce').offset().top-400;
$('#elm1').tinymce({
script_url : '../admin/jscripts/tinymce/tiny_mce.js',
translate_mode : true,
		language : "it",
		height : gH,
  width: '100%',
        theme : "advanced",
                        plugins : "autoresize,style,table,save,advimage,advlink,inlinepopups,searchreplace,contextmenu,paste,directionality,noneditable,visualchars,nonbreaking,xhtmlxtras,",
       theme_advanced_buttons1 : "|,save,|,cut,copy,paste,pastetext,pasteword,|,search,replace,|,bold,italic,underline,strikethrough,|,justifyleft,justifycenter,justifyright,justifyfull,|,bullist,numlist,|,fontsizeselect,forecolor",
		...
		theme_advanced_buttons3 : "|,tablecontrols,|,visualaid,|,styleselect",
...
autoresize_min_height: gH,
		content_css : "/admin/text_styles_admin2.css",
extended_valid_elements : "iframe[src|width|height|name|align],script[src|type]",
theme_advanced_styles : 'Title=titolo;Text=testo', external_link_list_url : "lists/link_list.asp",	external_image_list_url : "lists/image_list.asp"                });
openEditor();
}
...
//$('#structureContainerTable').resizable({ maxWidth: 400, minWidth: 280});
getOpened();
$('.loadSt').css('max-height',($(window).height()-300)+"px")
openEdit(actEdit);

setTimeout(function() {  setLimgs(); },700)
...
setLimgs()
	...
	//if ($('#selsections2').size()>0) getSlect('selsections2',maxlevs);
if ($('#selsections3').size()>0) getSlect('selsections3',maxlevs);
...
function getSlect(objdest, maxlev) {
...
setTimeout("getSlect('"+objdest+"',"+maxlev+");",500);
}});
} else {
